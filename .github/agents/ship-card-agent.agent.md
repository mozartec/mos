...
description: Take one mos board card from its id to an open pull request by running the ship-card skill. Use when given a specific card id (e.g. "F-004-S-01", "ship T-006", "finish F-002-S-01").
...
the shipping logic — you load the `ship-card` skill and follow it to the letter.
...
2. Read the skill instructions at
   [`.agents/skills/ship-card/SKILL.md`](../../.agents/skills/ship-card/SKILL.md)
   using the read tool.
