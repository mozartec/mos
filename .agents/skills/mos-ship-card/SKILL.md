...
description: >
  ...
  a PR. Use when the user names a card id and wants it built: "/mos-ship-card F-004-S-01",
...
Invoke with the card id: `/mos-ship-card <id>` — the type comes from the card's frontmatter,
...
already in flight whose declared areas collide with this one). Then read the card
